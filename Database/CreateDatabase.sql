...
BEGIN
    CREATE TABLE Products (
        ...
        UnitPrice DECIMAL(18,2) NOT NULL CHECK (UnitPrice >= 0),  -- Giá bán (phải >= 0)
...
BEGIN
    CREATE TABLE OrderItems (
        ...
        UnitPrice DECIMAL(18,2) NOT NULL CHECK (UnitPrice >= 0),  -- Đơn giá tại thời điểm bán (phải >= 0)
        LineTotal DECIMAL(18,2) NOT NULL CHECK (LineTotal >= 0),  -- Thành tiền = Quantity * UnitPrice (phải >= 0)
...
BEGIN
    CREATE TABLE Inventory (
        ...
        QuantityInStock INT NOT NULL DEFAULT 0 CHECK (QuantityInStock >= 0),  -- Số lượng tồn kho hiện tại (phải >= 0)
...
-- Index cho Orders.Status (lọc đơn hàng theo trạng thái)
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Orders_Status')
BEGIN
    CREATE INDEX IX_Orders_Status ON Orders(Status);
    PRINT '✓ Index IX_Orders_Status đã được tạo.';
END

-- Index cho Products.IsActive (lọc sản phẩm đang hoạt động)
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Products_IsActive')
BEGIN
    CREATE INDEX IX_Products_IsActive ON Products(IsActive);
    PRINT '✓ Index IX_Products_IsActive đã được tạo.';
END

-- Index cho Customers.Phone (tìm kiếm khách hàng theo số điện thoại)
...
BEGIN
    ...
    PRINT '✓ Index IX_Customers_Phone đã được tạo.';
END

-- Index cho Inventory.QuantityInStock (tìm sản phẩm tồn kho thấp)
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Inventory_QuantityInStock')
BEGIN
    CREATE INDEX IX_Inventory_QuantityInStock ON Inventory(QuantityInStock);
    PRINT '✓ Index IX_Inventory_QuantityInStock đã được tạo.';
END
