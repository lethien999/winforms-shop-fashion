...
namespace WinFormsFashionShop.Data.Repositories
{
    public class CustomerRepository : RepositoryBase, ICustomerRepository
    {
        ...
        public IEnumerable<Customer> GetAll()
        {
            ...
            using var cmd = new SqlCommand($"SELECT Id, CustomerName, Phone, Email, Address, IsActive, CreatedAt, UpdatedAt FROM {TableName}", conn);
        ...
        {
            using var conn = CreateOpenConnection();
            using var cmd = new SqlCommand($"SELECT Id, CustomerName, Phone, Email, Address, IsActive, CreatedAt, UpdatedAt FROM {TableName} WHERE Id=@Id", conn);
        ...
        public void Insert(Customer entity)
        {
            ...
            using var conn = CreateOpenConnection();
            using var cmd = new SqlCommand($"INSERT INTO {TableName} (CustomerName, Phone, Email, Address, IsActive, CreatedAt) VALUES (@CustomerName, @Phone, @Email, @Address, @IsActive, @CreatedAt); SELECT SCOPE_IDENTITY();", conn);
        ...
        public void Update(Customer entity)
        {
            if (entity == null) throw new ArgumentNullException(nameof(entity));
            using var conn = CreateOpenConnection();
            using var cmd = new SqlCommand($"UPDATE {TableName} SET CustomerName=@CustomerName, Phone=@Phone, Email=@Email, Address=@Address, IsActive=@IsActive, UpdatedAt=@UpdatedAt WHERE Id=@Id", conn);
            cmd.Parameters.AddWithValue("@CustomerName", entity.CustomerName);
            cmd.Parameters.AddWithValue("@Phone", (object?)entity.Phone ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@Email", (object?)entity.Email ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@Address", (object?)entity.Address ?? DBNull.Value);
